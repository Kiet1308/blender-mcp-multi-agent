...
- Keep a frame under about ten members, and nest frames when a stage outgrows that. A frame is a claim that its contents read as one step; past roughly ten nodes the claim stops being true and the frame degrades into a box. Split the stage into sub-frames parented to the outer one (`set_node_layout` `parent` accepts a NodeFrame), so the outer frame reads as a few named phases rather than a crowd. Count Group Input nodes toward the limit: the one-socket-per-Group-Input convention above multiplies them, so a group exposing eight parameters can need fourteen Group Input nodes and will blow the budget in a single stage.
