...
```powershell
Set-ExecutionPolicy Bypass -Scope Process -Force; & ([scriptblock]::Create(([string](irm https://raw.githubusercontent.com/newo-ether/blender-mcp/v1.16.1/install.ps1)).TrimStart([char]0xFEFF))) -ReleaseTag v1.16.1
```
...
5. installs into a versioned environment such as
   `%LOCALAPPDATA%\BlenderMCP\venv-1.16.1`;
...
```powershell
py -3 -m venv .venv
.\.venv\Scripts\python.exe -m pip install .\blender_mcp-1.16.1-py3-none-any.whl
```
...
python -m pip install ./blender_mcp-1.16.1-py3-none-any.whl
```
