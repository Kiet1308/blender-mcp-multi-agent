...
class InstanceConnectionManager:
    ...
    def _merge_live_handshake(
        ...
        """Refresh registry-derived public state from an authoritative probe."""
        claim = handshake.get("claim") or {}
        owner_id = claim.get("client_id", "")
        claim_alive = float(claim.get("expires_at", 0.0) or 0.0) > time.time()
        public["allow_ai_control"] = bool(handshake.get("allow_ai_control", True))
        public["busy"] = bool(handshake.get("busy", False))
        public["claim_owner"] = claim.get("owner_label", "") if claim_alive else ""
        public["claim_expires_at"] = claim.get("expires_at") if claim_alive else None
        if not public["allow_ai_control"]:
            status = "manual"
        elif public["busy"]:
            status = "busy"
        elif claim_alive:
            status = (
                "claimed_by_this_client"
                if owner_id == self.client_id
                else "claimed_by_other_client"
            )
        else:
            status = "ready"
        public["status"] = status
        public["may_auto_claim"] = status in {"ready", "claimed_by_this_client"}
        public["next_action"] = _next_action(status)
    ...
    def list_instances(self, *, validate_live: bool = True) -> list[dict[str, Any]]:
        ...
        for item in discovered:
            ...
            if validate_live and item.status not in {"busy", "protocol_mismatch"}:
                connection = None
                try:
                    ...
                    self._merge_live_handshake(public, handshake)
                    if item.status == "stale":
                        public["heartbeat_age"] = 0.0
                except BlenderMCPError as error:
                    public["status"] = (
                        "protocol_mismatch"
                ...
                public["may_auto_claim"] = public["status"] in {
                    "ready",
                    "claimed_by_this_client",
                }
                public["next_action"] = _next_action(public["status"])
    ...
    def claim(
        ...
        *,
        ...
        with self._state_lock:
            ...
            record = item.record
            if not record.get("host") or not record.get("port"):
                raise BlenderMCPError(
                    "instance_stale",
                    "Blender instance registration is incomplete or stale",
                    retryable=True,
                )
            ...
            same_target = bool(
                self.active_record
                and self.active_record.get("instance_id") == instance_id
            )
            if same_target:
                if self.active is not None and self.active is not connection:
    ...
    def auto_select(self) -> Any:
        with self._state_lock:
            ...
            owned = [
                ...
                if (item.record.get("claim") or {}).get("client_id") == self.client_id
                and float((item.record.get("claim") or {}).get("expires_at", 0.0) or 0.0) > now
            ]
            if len(owned) == 1:
                self.claim(owned[0].record["instance_id"])
                return self.active
            if len(owned) > 1:
                raise BlenderMCPError(
                    "multiple_instances_require_selection",
                    "This MCP client has claims on more than one Blender instance; select one explicitly",
                    details={"instance_ids": [item.record["instance_id"] for item in owned]},
                )
            ready = [item for item in records if item.status == "ready"]
            if not ready and len(records) == 1 and records[0].status == "stale":
                self.claim(records[0].record["instance_id"])
                return self.active
            if len(ready) != 1:
                if len(ready) > 1:
                    raise BlenderMCPError(
    ...
    def active_summary(self) -> dict[str, Any]:
        with self._state_lock:
            ...
            return {
                ...
                "handoff_action": "release_blender_instance",
            }
    ...
    def release(self, *, ignore_errors: bool = False) -> dict[str, Any]:
        with self._state_lock:
            ...
            try:
                ...
                result = {
                    "released": False,
                    "reason": "release_transport_failed",
                    "error": str(error),
                }
            finally:
                self.invalidate()
